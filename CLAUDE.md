...
- **HTML 报告输出路径是临床报告而非仪表板**：`src/report/exportReport.ts` 在 2026-06-04 第一轮美化和 2026-06-05 临床报告风格重构之后，输出结构由"卡片堆叠"升级为"封面 + 摘要 + TOC + 8 段章节 + 公式 tip + 严重度分布图 + caption/footnote + A4 打印页眉页码"。新增视觉/CSS 块（`.cover` / `.exec-summary` / `.toc` / `.formula-tip` / `.dist-chart` / `.table-caption` / `.footnotes` / `.legend` / `.historical-banner` / `.spacing-bar` / `.severity-{high,medium,low}` / `.organ-list-details`）必须有对应 `tests/imagingLogic.test.ts` source-grep 断言保护。`.remap-banner` 顶部警告条 CSS 已在 2026-06-07 清理（remap 信息改走 `.tag.tag-progress` + 摘要列表内联文字，更对齐 6-05 临床报告风格）；不要复活这个类，改 UI 表达。改 exportReport.ts 时**不要**回退到 6-04 之前那种"工程 dump"风格。后续若新增 validation 字段（`remap_applied` / `taxonomy_match` / `dataset_hint` / `historical` / `label_taxonomy` / `quantification`），必须同时改 `inferenceClient.ts normalizeValidation` 白名单 + `exportReport.ts` 模板 + `tests/imagingLogic.test.ts` source-grep 三处
...
- **2026-06-06 演示当天收口不变量**（已落地）：
  - **B1 SSE 进度百分比不再随资源快照心跳回退到旧值**：`src/main.tsx:infereneTimeline` 的百分比追踪以 `event.percent` 存在为强信号，未带 `percent` 的心跳事件不覆盖当前进度；`tests/imagingLogic.test.ts` 守护。**2026-06-06 `76bb1ff` 补完**：6-06 `23e0c4d` 只在文档/commit 写了此条，源码实际未做；同日 `76bb1ff` 真正实现 `parsed.heartbeat && parsed.progress === 0` 守护并补 source-grep 守护。
  - **B2 取消 job 后 SSE 不会再有 progress 事件**：`cancel_job()` 在 `EventSourceHandler` 关闭后写入取消状态；前端不把 cancel 后的心跳误显示为"还在跑"。**2026-06-06 `76bb1ff` 补完**：6-06 `23e0c4d` 虚标；同日 `76bb1ff` 真正实现 `inferenceStatusRef` 镜像 + SSE onmessage 入口 `status === "cancelled"` 早退。
  ...
  - **B4 浏览器 SSE 事件源具备基础异常重试**：`createInferenceEventSource` 暴露 `onretry` / `retryCount` 字段；单次断连后自动退避重连（200ms→2s 指数退避，最多 3 次），不再让网络抖动直接打断演示。**2026-06-06 `76bb1ff` 补完**：6-06 `23e0c4d` 虚标；同日 `76bb1ff` 真正抽出 `src/inference/createInferenceEventSource.ts` 工具并接入 main.tsx。
...
# 测试
# ⚠️ npm test 必须在已激活 venv 的 shell 里跑（PowerShell 默认 `python` 指向系统 Python，
#    系统 Python 没装 nibabel，tests/backendState.test.py 会在 import nibabel 时挂）。
#    没激活时跑：`& "D:\BME2026\BME_CT_Seg\nnunet_env\Scripts\Activate.ps1"`，或单测直接用
#    `& "D:\BME2026\BME_CT_Seg\nnunet_env\Scripts\python.exe" tests/<name>.test.py`。

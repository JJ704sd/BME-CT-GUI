...
## 2026-06-27 决策：暂时搁置，4 项候选待需要时启动

- **决策**：本日（2026-06-27）完成 14 天空窗期文档巡检（commit `cf382db` 8 份 md 同步 + commit `a6dde28` AGENTS.md 同步）后，4 项候选（中/中高优先级）暂不推进，等需要时再启动。
- **涵盖范围**：上面 1-6 项中高/中优先级候选，**不包括**第 7 项"文档与验收口径再同步"——这次 6-27 巡检本身已算第 7 项的最近一次落地。
- **下次启动触发条件**（任一满足即可考虑重新评估）：
  1. 服务器窗口可用且想确认 AMOS/FLARE 显式 taxonomy 基线 → 启动候选 3
  2. 用户报告 cache hit 仍显示"无历史验证摘要" → 启动候选 4
  3. runbook 7 步约束在重构中被破坏 → 启动候选 5
  4. 服务器 5-fold ensemble 基础设施就绪 → 启动候选 2
  5. AMOS 高分辨率推理在演示场景频繁超时 → 启动候选 1
- **不做的事**：
  - 不在 6-27 这次做 4 项候选的预研 / 设计稿 / 排期
  - 不在 `.planning/` 下新建 4 个候选子主题（避免提前占位）
  - 不修改本文件原有 4 项候选的解释与优先级排序
- **回看入口**：`git log --grep="6-27 doc consistency pass"` 看这次巡检的两次 commit；如需重开 4 项候选任一项，从上面 5 条触发条件里选匹配的描述，重新评估优先级即可。
- **本决策与历史关系**：本文件原"更新日期：2026-06-06"标注的 4 项候选与本决策同时存在；6-27 这次决策**不撤销**原候选描述，只在末尾标注"暂不推进"状态，与原文档结构兼容叠加。

...
- **演示启动脚本化**验收：`tools/start_local_demo.py` 一行启动：setenv + spawn backend（uvicorn 127.0.0.1:8000）+ frontend（vite dev 127.0.0.1:5173）+ 启动后采样 `/api/samples`（最多 15s）校验 4 例参考病例（AMOS 0117 / FLARE22 Tr 0009 / WORD / AbdomenCT-1K）已就绪 + 失败时打印 runbook 回退命令。`docs/demo-day-checklist.md` 是配套一屏卡片。`python tools/start_local_demo.py` 真启后端 + 前端 smoke test 2026-06-06 通过。
